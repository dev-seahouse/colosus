...
RUN yarn nx run cron-syncronize-model-portfolio-wk-models:build:production --verbose && ls -l

FROM --platform=$BUILDPLATFORM node:18.17.1-alpine3.18

RUN echo "*/30 * * * * /usr/local/bin/node /app/apps/cron-jobs/cron-syncronize-model-portfolio-wk-models/main.js" >> /var/spool/cron/crontabs/root
...
COPY --chown=colossus:colossus --from=base /app/node_modules/@bambu/colossus/prisma/central-db ./node_modules/@bambu/colossus/prisma/central-db
...
COPY --chown=colossus:colossus --from=builder /app/dist/apps ./apps

RUN ls -l

# CMD ["node", "app/apps/cron-jobs/cron-syncronize-model-portfolio-wk-models/main.js"]
