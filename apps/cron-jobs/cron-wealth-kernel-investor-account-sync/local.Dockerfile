...
RUN yarn nx run cron-wealth-kernel-investor-account-sync:build:production --verbose && ls -l

FROM --platform=$BUILDPLATFORM node:18.17.1-alpine3.18

RUN echo "*/30 * * * * /usr/local/bin/node /app/apps/cron-jobs/cron-wealth-kernel-investor-account-sync/main.js" >> /var/spool/cron/crontabs/root
...
# CMD ["node", "app/apps/cron-jobs/cron-wealth-kernel-investor-account-sync/main.js"]
