...
export abstract class WealthKernelPerformanceApiRepositoryServiceBase {
...
@Injectable()
export class WealthKernelPerformanceApiRepositoryService
  implements WealthKernelPerformanceApiRepositoryServiceBase
{
  private readonly logger: Logger = new Logger(
    WealthKernelPerformanceApiRepositoryService.name
  );
  ...
  public async List(
    ...
    try {
      ...
      const url = `${this.baseUrl}/performance?${queryParams.toString()}`;
      const source = this.httpService.get<
        BrokerageIntegrationServerDto.IBrokerageIntegrationPerformanceDto[]
      >(url, {
