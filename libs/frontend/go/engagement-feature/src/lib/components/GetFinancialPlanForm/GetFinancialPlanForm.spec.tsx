...
import GetFinancialPlanForm from './GetFinancialPlanForm';
...
const submitForm = async () =>
  userEvent.click(
    screen.getByRole('button', { name: /send me the financial plan/i })
  );
...
describe(
  ...
  () => {
    ...
    beforeEach(() => {
      render(<GetFinancialPlanForm />);
    });

    test('should display error message if email is not provided', async () => {
      await fillNameInput();
      await agreeToTerms();
      await submitForm();
