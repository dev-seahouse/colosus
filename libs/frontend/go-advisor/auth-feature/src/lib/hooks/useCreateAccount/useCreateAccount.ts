...
import type { ConnectAdvisorCreateAccountRequestDto } from '@bambu/api-client';
import { useMutation } from '@tanstack/react-query';
import type { AxiosError } from 'axios/index';

export const createAccount = async (
  req: ConnectAdvisorCreateAccountRequestDto
) => {
  const connectAdvisorAuthApi = new ConnectAdvisorAuthApi();

  return connectAdvisorAuthApi.createAccount(req).then((res) => res.data);
};
